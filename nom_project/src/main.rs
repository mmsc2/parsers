...
// immediate for cases when you access with a single aleatory value cast(89898989, felt*)
// value when you have an offset that is not immediate
...
pub fn parse_value(input: &str) -> IResult<&str, ValueAddress> {
    ...
    println!("");
    println!("indirection_level: {:?}", indirection_level);
...
mod tests {
    ...
    #[test]
    fn parse_value_with_no_inner_deref_and_immediate() {
        let value = "[cast(ap + 1, felt)]";
        let parsed = parse_value(value);
